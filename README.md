...
投資系システムは現状以下の3階層構成になっている。
依存関係は上から順、下の階層は上の階層に依存しており、逆転は起こらないように構築している。

- InvestSupporter
  - EC2、常時起動
  - JquantsAPIからの情報取得、整形保存
  - 財務データ、株価データの API エンドポイント提供
- invest-funcs（private）
  - CloudFlare Workers、サーバレス
  - 証券口座からの情報取得、整形して返却する API エンドポイント提供
  - プッシュ通知送信
- スプレッドシート
  - 上記2つの API エンドポイントを呼び出して、投資情報を可視化するフロントエンド
  - IMPORTDATA 関数と GAS を利用
